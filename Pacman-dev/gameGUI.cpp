/*    Pacman- Two player AI based game
    Copyright (C) <2014>  <Shagun Akarsh>
...
class GUI
{
        ...
        {
            plays(3);
            renderText(160,210,"iFest IIT Roorkee",textColor,1);
            renderText(235,240,"presents",textColor,1);
            SDL_Flip(screen);
            CheckForEvents();
            SDL_Delay(1000);
            SDL_FillRect(screen, NULL, clearColor);
            renderText(210,210,"P A C M A N",textColor,1);
            SDL_Flip(screen);
            CheckForEvents();
            SDL_Delay (1000);
            SDL_FillRect(screen, NULL, clearColor);
        ...
        public:
        void displayGUI(char map[ROWS][COLS],int move1,int move2,int tc,Players p1,Players p2,Ghosts ghost[N_GHOSTS],int moves,int end)
        {
             for(int k=0;k<=6;k++)
            {
                CheckForEvents();
                ...
                { 
                        ...
                        switch(map[i][j])
                        {
                                 ...
                                 case '1':
                                      ...
                                      {
                                      switch(move1/*(move1==STAY)?play1.direction:move1*/)
                                      {
                                                   case UP:
                                                   ...
                                                   break;
                                                   case STAY:
                                                   SDL_BlitSurface(player1,&clip[2][3],screen,&off);
                                                   break;
                                      }
                                      }
                                 break;
                                 case '2':
                                      ...
                                      {
                                      switch(move2/*(move2==STAY)?play2.direction:move2*/)
                                      {
                                                   case UP:
                                                   ...
                                                   break;
                                                   case STAY:
                                                   SDL_BlitSurface(player2,&clip[2][3],screen,&off);
                                                   break;
                                      }
                                      }
                                 break;
                ...
                if(k==6)
                {
                     ...
                     if(p1.health<play1.health&&!pd1&&p2.health<play2.health&&!pd2)
                    {
                         ...
                         plays(2);
                         displayGUI(map,move1,move2,tc,p1,p2,ghost,moves,end);
                         pd1=0;pd2=0;
                     }
                     else
                     {
